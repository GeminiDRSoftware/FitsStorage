                ...
                script {
                    ...
                    def postgres = docker.image('postgres:12').withRun(" --network fitsstorage-release-jenkins --name fitsdata-release-jenkins -e POSTGRES_USER=fitsdata -e POSTGRES_PASSWORD=fitsdata -e POSTGRES_DB=fitsdata") { c ->
                        def archive = docker.image("gemini/archive:release-jenkins").withRun(" --network fitsstorage-release-jenkins --name archive-release-jenkins -e FITS_DB_SERVER=\"fitsdata:fitsdata@fitsdata-release-jenkins\" -e TEST_IMAGE_PATH=/tmp/archive_test_images -e TEST_IMAGE_CACHE=/tmp/cached_archive_test_images -e CREATE_TEST_DB=False -e BLOCKED_URLS=\"\" -e PYTHONPATH=/opt/FitsStorage:/opt/DRAGONS") { a->
                            try {
                                docker.image('gemini/fitsarchiveutils:release-jenkins').inside(" -v /data/pytest_tmp:/tmp  --network fitsstorage-release-jenkins -e STORAGE_ROOT=/tmp/jenkins_pytest/dataflow -e FITS_DB_SERVER=\"fitsdata:fitsdata@fitsdata-release-jenkins\" -e PYTEST_SERVER=http://archive-jenkins -e TEST_IMAGE_PATH=/tmp/archive_test_images -e TEST_IMAGE_CACHE=/tmp/cached_archive_test_images -e BLOCKED_URLS=\"\" -e CREATE_TEST_DB=False -e PYTHONPATH=/opt/FitsStorage:/opt/DRAGONS") {
