...
def upgrade(migrate_engine):
    ...
    with migrate_engine.connect() as connection:
        ...
        connection.execute("update diskfile set datafile_timestamp=to_date(substring(filename from 5 for 8), 'YYYYMMDD') where filename like '[A-Z][A-Z][A-Z][A-Z]_20[0-9][0-9][0-1][0-9][0-3][0-9]%'")
