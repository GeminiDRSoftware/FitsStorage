...
for diskfileid in diskfileids:
  ...
  for fe in fileelements:
    ...
    if((filename == dbfilename) and (ccrc == dbccrc) and (tapeid not in tapeids)):
  ...
  if(len(tapeids) < options.mintapes):
    ...
    sumfiles += 1
    logger.info("*** File %s - %s needs to go to tape, it is on %d tapes: %s" % (fullpath, dbccrc, len(tapeids), tapeids))
  else:
    logger.info("File %s - %s is OK, it already is on %d tapes: %s" % (fullpath, dbccrc, len(tapeids), tapeids))
