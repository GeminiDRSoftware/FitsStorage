...
#!/bin/bash
docker run --name createtables --network fitsstorage -e FITS_DB_SERVER="fitsdata:fitsdata@postgres-onsite" --rm -it fitsstorageutils:2022-1 python3 ./fits_storage/scripts/create_tables.py
docker run --name createtables --network fitsstorage -e FITS_DB_SERVER="fitsdata:fitsdata@postgres-archive" --rm -it fitsstorageutils:2022-1 python3 ./fits_storage/scripts/create_tables.py
