...
# Start Archive
docker run --network fitsstorage-jenkins --name archive-jenkins -e FITS_DB_SERVER="fitsdata:fitsdata@fitsdata-jenkins" -e TEST_IMAGE_PATH=/tmp/archive_test_images -e TEST_IMAGE_CACHE=/tmp/cached_archive_test_images -e CREATE_TEST_DB=False -e PYTHONPATH=/opt/FitsStorage:/opt/DRAGONS:/opt/GeminiCalMgr:/opt/FitsStorageDB -d gemini/archive:jenkins

# Run Table init and tests
docker run --name archive-tester  --network fitsstorage-jenkins -e FITS_DB_SERVER="fitsdata:fitsdata@fitsdata-jenkins" -e PYTEST_SERVER=http://archive-jenkins -e TEST_IMAGE_PATH=/tmp/archive_test_images -e TEST_IMAGE_CACHE=/tmp/cached_archive_test_images -e CREATE_TEST_DB=False -e PYTHONPATH=/opt/FitsStorage:/opt/DRAGONS:/opt/GeminiCalMgr:/opt/FitsStorageDB -d gemini/fitsarchiveutils:jenkins bash -c "python3 fits_storage/scripts/create_tables.py; mkdir -p /tmp/archive_test_images; mkdir -p /tmp/cached_archive_test_images; cd /opt/FitsStorage && pytest tests "
...
docker network rm fitsstorage-jenkins

docker image rm gemini/archive:jenkins
docker image rm gemini/fitsarchiveutils:jenkins
docker image rm gemini/epelhack:jenkins
