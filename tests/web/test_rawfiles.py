...
@pytest.mark.usefixtures("rollback")
def test_rawfiles(session, monkeypatch):
    mock_context = MockContext(session, method='GET', usagelog=None)

    def _mock_get_context(initialize=True):
        return mock_context

    monkeypatch.setattr(fits_storage.web.rawfiles, "get_context", _mock_get_context)
    ...
    session.add(h)
    session.flush()
    rf = File('rawfile.fits')
    session.add(rf)
    session.flush()
    rdf = DiskFile(rf, 'rawfile.fits', '')
    rdf.canonical = True
    session.add(rdf)
    session.flush()
    rh = Header(rdf)
    rh.ut_datetime = datetime.now()
    session.add(rh)
    session.flush()
    p = Provenance(datetime.now(), 'rawfile.fits', 'md5', 'primitive')
    p.diskfile_id = df.id
    session.add(p)
    session.flush()
    ...
    assert('<td>rawfile.fits</td>' in mock_context.resp.stuff)
