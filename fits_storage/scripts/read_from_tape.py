...
if __name__ == "__main__":
    ...
    parser.add_option("--requester", action="store", dest="requester", help="Read files for a given requester")
    ...
    try:
        ...
        # OK, now we need to get a list of the filenums (tapewrites) that contain files we want
        query = session.query(TapeWrite).select_from(Tape, TapeWrite, TapeFile, TapeRead)
        query = query.filter(Tape.id == TapeWrite.tape_id).filter(TapeWrite.id == TapeFile.tapewrite_id)
        query = query.filter(Tape.active == True).filter(TapeWrite.suceeded == True)
        ...
        query = query.distinct().order_by(TapeWrite.filenum)
        tws = query.all()
        ...
        for tw in tws:
            ...
            logger.debug("Reading from filenumber %d" % filenum)
